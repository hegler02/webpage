...
export default defineConfig({appType:'mpa',server:{allowedHosts:['terminal.local']},plugins:[{name:'public-profile-routes',configureServer(server){server.middlewares.use((req,res,next)=>{const path=req.url.split('?')[0];const route=manifest.routes.find(r=>r.public_path===path);if(route)req.url='/pages/profile/'+route.physical_path+req.url.slice(path.length);next();});}}]});
