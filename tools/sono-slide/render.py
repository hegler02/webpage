...
def section(n,cls,body,motion='leaf'):
 part=1 if n<5 else 2 if n<13 else 3
 spine=f'<span class="sleeve-spine" aria-hidden="true">MIRINAEMAN RECORDS / SIDE {SIDES[part-1]} / {n:02}</span>'
 return f'<section id="slide-{n:02}" class="slide {cls}" data-part="{part}" data-motion="{motion}" tabindex="-1">{spine}{body}<span class="slide-index">{n:02} / {SCENE_COUNT}</span></section>'
def side(letter,subtitle):
 return f'<div class="side-stamp reveal"><span>SIDE</span><strong>{letter}</strong><span>{subtitle}</span></div>'
slides=[]
slides.append(section(1,'hero paper',f'<div class="hero-copy">{label("SIDE A / A PERSONAL MEDIUM")}{t("s1_title","h1","display reveal")}{t("s1_lead","p","lead reveal")}<div class="signature">{t("s1_speaker")}</div></div><figure class="record-art reveal"><figcaption><span>MIRINAEMAN RECORDS</span><span>VOL. 01</span></figcaption><div class="cover-type" aria-hidden="true">PRIVATE<br><i>MUSIC.</i></div><img src="assets/album-home.webp" width="960" height="540" alt="내가 집으로 가는 줄 알았지 — 실제 발표 작품 이미지"><div class="record-label"><span>MY STORY. MY SONG.</span><b>SIDE A</b></div></figure>', 'sleeve'))
slides.append(section(2,'manifesto',label('SIDE A / LINER NOTES')+t('s2_title','h2','manifesto-title reveal')+'<div class="statement-rule"></div>'+t('s2_lead','p','statement-copy reveal'),'statement'))
slides.append(section(3,'shift paper',head(3,'SIDE A / PUBLIC → PRIVATE')+'<div class="content comparison two"><article class="panel reveal public"><span class="panel-kicker">PUBLIC / LISTEN</span>'+t('listen','h3')+t('public_detail')+'</article><article class="panel reveal private"><span class="panel-kicker">PRIVATE / SPEAK</span>'+t('speak','h3')+t('private_detail')+'</article></div>','spread'))
slides.append(section(4,'liner',head(4,'SIDE A / A CHANGE OF CENTER')+'<div class="content three">'+''.join(panel(i,f's4_c{i}_t',f's4_c{i}_p','featured' if i==2 else '') for i in range(1,4))+'</div>','spread'))
slides.append(section(5,'chapter paper',side('B','DESIGN THE SONG')+'<div class="chapter-copy">'+label('LYRICS / GENRE / VOCAL / PURPOSE')+t('s5_title','h2','chapter-title reveal')+'<div class="chapter-footer"><span>THE SONGBOOK</span><span>05—12</span></div></div>','sleeve'))
slides.append(section(6,'paper liner',head(6,'SIDE B / FOUR DECISIONS')+'<div class="content four question-cards">'+''.join(panel(i,f's6_c{i}_t',f's6_c{i}_p') for i in range(1,5))+'</div>','spread'))
slides.append(section(7,'media-slide',head(7,'SIDE B / THE PRODUCTION BRIEF')+'<div class="content media-grid"><div class="media-copy reveal"><span class="wordmark">SUNO<span>↗</span></span>'+t('s7_p','p','lead')+'</div>'+screen('suno-start','Suno · Start')+'</div>'))
slides.append(section(8,'media-slide',head(8,'SIDE B / CREATE → LISTEN → REFINE')+'<div class="content media-grid reversed">'+screen('suno-revise','Suno · Create')+'<div class="media-copy reveal">'+t('s8_lead','p','lead')+'<div class="revision-loop" aria-hidden="true">01 → 02 → 03</div></div></div>'))
arrangement=''
for i,name in enumerate(['Verse','Pre-Chorus','Chorus','Bridge'],1):
 arrangement+=f'<article class="arrange-cell {"chorus" if i==3 else ""}"><div class="track-label"><span class="ordinal">{i:02}</span><h3>{name}</h3></div><div class="track-clip reveal" aria-hidden="true"><span>{name.upper()}</span></div>'+t(f's9_c{i}_p')+'</article>'
slides.append(section(9,'arrangement-slide',head(9,'SIDE B / SONG ARRANGEMENT')+'<div class="content arrangement">'+arrangement+'</div>'+t('structure_note','p','takeaway'),'track'))
slides.append(section(10,'paper session',head(10,'SIDE B / THE SESSION SHEET')+'<div class="content brief-grid"><div class="brief-anchor reveal"><span>SESSION<br>NOTES<span class="anchor-dot">.</span></span>'+t('brief_note')+'</div><ol class="brief-list">'+''.join(f'<li class="reveal"><span>{i:02}</span>'+t(f's10_{i}')+'</li>' for i in range(1,6))+'</ol></div>','notes'))
slides.append(section(11,'liner',head(11,'SIDE B / PURPOSE SHAPES FORM')+'<div class="content four purpose">'+''.join(panel(i,f's11_c{i}_t',f's11_c{i}_p') for i in range(1,5))+'</div>','spread'))
slides.append(section(12,'paper liner',head(12,'SIDE B / THREE WAYS TO SPEAK')+'<div class="content three modes">'+''.join(f'<article class="panel reveal"><span class="ordinal">0{i}</span><h3>{name}</h3>'+t(f's12_c{i}_p')+'</article>' for i,name in enumerate(['Instrumental','Vocal','Hybrid'],1))+'</div>'+t('s12_lead','p','takeaway'),'spread'))
slides.append(section(13,'workflow',head(13,'SIDE C / FROM IDEA TO RELEASE')+'<div class="content media-grid"><ol class="workflow-list">'+''.join(f'<li class="reveal"><span>{i:02}</span>'+t(f's13_{i}')+'</li>' for i in range(1,6))+'</ol>'+screen('suno-publish','Suno · Library')+'</div>','notes'))
albums=[]
for i,work in enumerate(d['release_works'],1):
 albums.append(f'<a class="album reveal" href="{escape(work["url"],quote=True)}" target="_blank" rel="noopener noreferrer"><figure><img src="{escape(work["image"],quote=True)}" width="{work["width"]}" height="{work["height"]}" alt="{escape(ko[work["title_key"]])}" loading="lazy"><span class="album-number">{i:02}</span></figure><div class="album-copy">'+t(work['title_key'],'h3')+t(work['description_key'])+t('open_work','span','album-open')+'</div></a>')
slides.append(section(14,'release paper',head(14,'SIDE C / THE RELEASE SHELF')+'<div class="content four albums">'+''.join(albums)+'</div>'+t('case_note','p','takeaway'),'shelf'))
slides.append(section(15,'agent-slide',head(15,'SIDE C / SOUND DESIGN LAB')+'<div class="content media-grid"><div class="media-copy reveal"><div class="agent-mark" aria-hidden="true">S.D.L.</div>'+t('lab_note','p','lead')+'<a class="cta" href="https://chatgpt.com/g/g-69f690a099f881918507ce23174fc75e-sound-design-lab-v2" target="_blank" rel="noopener noreferrer">'+t('s15_link','span')+'<span aria-hidden="true">↗</span></a></div>'+screen('sound-lab','Sound Design Lab V2')+'</div>'))
slides.append(section(16,'practice paper',head(16,'SIDE C / YOUR TURN')+'<div class="content practice-sheet reveal"><span class="sheet-label">WRITE YOUR CHORUS</span>'+t('practice_prompt','p','practice-quote')+t('practice_step','p','lead')+'<div class="sheet-line" aria-hidden="true"></div><div class="practice-tags"><span>STORY</span><span>MESSAGE</span><span>GENRE</span><span>VOCAL</span></div></div>','notes'))
slides.append(section(17,'final',label('SIDE C / THE SONG IS YOURS')+t('s17_title','h2','final-title reveal')+t('s17_lead','p','final-lead reveal')+'<div class="final-footer"><span>Q&amp;A</span><a href="https://mirinaeman.com/archive/">'+t('archive','span')+'</a><a href="https://mirinaeman.com/profile/">'+t('profile','span')+'</a></div>','sleeve'))
...
<script type="application/ld+json">{json.dumps(graph,ensure_ascii=False)}</script><script id="deck-settings" type="application/json">{json.dumps({'parts':d['parts'],'sides':SIDES,'i18n':d['i18n']},ensure_ascii=False)}</script>
