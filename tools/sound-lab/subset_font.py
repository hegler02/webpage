...
R=Path(__file__).resolve().parents[2];P=R/'pages/sound_design_lab'
a=argparse.ArgumentParser();a.add_argument('--source',type=Path,required=True);args=a.parse_args()
text=(P/'index.html').read_text()+(P/'presentation.js').read_text()+(P/'workbook.js').read_text()+(P/'motion.js').read_text()+''.join(chr(n) for n in range(32,127))+'0123456789←→↗↓×·“”‘’—'
...
for name in f['name'].names:
 if name.nameID in {1,3,4,6,16}:name.string=('MirinaeLabSans' if name.nameID==6 else 'Mirinae Lab Sans').encode(name.getEncoding(),errors='replace')
f.flavor='woff2';out=P/'assets/lecture-sans.woff2';f.save(out)
record={'family':'Mirinae Lab Sans','derived_from':'Pretendard','source_sha256':hashlib.sha256(args.source.read_bytes()).hexdigest(),'license':'SIL Open Font License 1.1','license_file':'font-license.txt','source_author':'Kil Hyung-jin','source_project':'https://github.com/orioncactus/pretendard','delivery_sha256':hashlib.sha256(out.read_bytes()).hexdigest(),'delivery_bytes':out.stat().st_size,'scope':'Characters in the complete lecture HTML and controller plus ASCII; variable weights preserved.'}
